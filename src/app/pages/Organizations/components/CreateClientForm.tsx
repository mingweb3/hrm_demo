...
import { createOrgnizationFn } from '@/apis/organization.api'
...
const initFormData: IOrganizationForm = {
  companyName: '',
  buzRegNum: '',
  buzAddress: '',
  billAddress: '',
...
export const CreateClientForm: React.FC = () => {
  ...
  const {
    mutate: createOrgnization,
    isLoading,
    error
  } = useMutation((formData: IOrganizationForm) => createOrgnizationFn(formData), {
    onSuccess: data => {
      reset(initFormData)
      enqueueSnackbar(t(messages.CreateNewSuccess()), { variant: 'success' }) // ADD Notify
      navigate(`/admin/organizations/${data.attributes.UUID}/details`)
    }
  })
  ...
  return (
    <StyledForm>
      <div className="flex flex-col gap-4">
        <WToolBox className="p-4">
          <div className="flex items-center justify-end gap-2">
            <Button onClick={handleSubmit(onSubmit)} className="min-w-[92px]" variant="primary">
              {!isLoading && <span className="uppercase">{t(messages.Create())}</span>}
        ...
        <div className="frm-ct flex flex-col gap-4">
