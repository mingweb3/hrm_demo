...
import { createWorkPlaceFn } from '@/apis/organization.api'
...
export const CreateLocationForm: React.FC<CreateLocationFormProps> = ({ orgID, onClose }) => {
  ...
  const {
    ...
    isLoading,
    error
  } = useMutation((formData: IWorkPlaceForm) => createWorkPlaceFn(formData), {
    onSuccess: () => {
      reset(initFormData)
  ...
  return (
    <StyledForm>
      <div className="flex flex-col gap-4 h-full">
        ...
        <WToolBox className="p-4 mt-auto">
          <div className="flex items-center justify-end gap-2">
            <Button onClick={handleSubmit(onSubmit)} className="min-w-[92px]" variant="primary">
              {!isLoading && <span className="uppercase">{t(messages.Create())}</span>}
              {isLoading && <LoadingBtn />}
